...
export default class Car3D {
  constructor(
    ...
    // --- OPTIMIZATION: Object Pool for Skidmarks ---
  ...
  createSkidMarks() {
    ...
    this.wheels.forEach((wheel) => {
      if (!wheel.isFront && wheel.group) {
        ...
        wheel.group.getWorldPosition(pos);

        let m = this.skidMarks[this.skidIndex];
        m.mesh.position.copy(pos);
        m.mesh.position.y = 0.11;
        m.mesh.rotation.y = this.angle;
  ...
  updateSkidMarks() {
    for (let i = 0; i < this.skidPoolSize; i++) {
      ...
      if (m.life > 0) {
        m.life -= 0.01;
        m.mesh.scale.setScalar(Math.max(0, m.life));
  ...
  loadRealModel(carColor, modelPath) {
    ...
    // Use the dynamic path instead of a hardcoded string
    loader.load(modelPath, (gltf) => {
      ...
      // The 3D model MUST have these exact names in Blender
      let frontLeft = carModel.getObjectByName("wheel-front-left");
      let frontRight = carModel.getObjectByName("wheel-front-right");
      let backLeft = carModel.getObjectByName("wheel-back-left");
      let backRight = carModel.getObjectByName("wheel-back-right");

      if (frontLeft) this.wheels.push({ group: frontLeft, isFront: true });
      if (frontRight) this.wheels.push({ group: frontRight, isFront: true });
      if (backLeft) this.wheels.push({ group: backLeft, isFront: false });
      if (backRight) this.wheels.push({ group: backRight, isFront: false });
  ...
  update(input, canDrive) {
    ...
    if (canDrive && !this.isAI && !this.finished) {
      ...
      if (
        ...
        this.nitrous += 0.15;
        if (this.nitrous > 100) this.nitrous = 100;
      }
    }

    if (!this.isAI) {
      if (this.boostTimer > 0 || isUsingNos) {
        document.getElementById("speed-lines").classList.add("boosting");
      } else {
        document.getElementById("speed-lines").classList.remove("boosting");
      }
    }
    ...
    if (canDrive && !this.finished) {
      if (!this.isAI) {
        if (input.isPressed(["ArrowUp", "KeyW"]))
          this.speed += this.config.accel;
        if (input.isPressed(["ArrowDown", "KeyS"]))
          this.speed -= this.config.accel;
        if (input.isPressed(["Space"])) this.speed *= 0.8;

        if (Math.abs(this.speed) > 0.1) {
          turnDir = this.speed > 0 ? 1 : -1;
          if (input.isPressed(["ArrowLeft", "KeyA"])) {
            this.angle += this.config.handling * turnDir;
            turnDir = 1;
          } else if (input.isPressed(["ArrowRight", "KeyD"])) {
            this.angle -= this.config.handling * turnDir;
            turnDir = -1;
          } else turnDir = 0;
        }
    ...
    if (canDrive && !this.finished) {
      if (this.isAI) {
        if (
          this.lastAiTurnDir &&
          this.lastAiTurnDir !== 0 &&
          Math.abs(this.speed) > this.config.maxSpeed * 0.7
        )
          isSkidding = true;
      } else {
        const isTurning =
          input.isPressed(["ArrowLeft", "KeyA"]) ||
          input.isPressed(["ArrowRight", "KeyD"]);
        const isBraking = input.isPressed(["Space"]);
        const isFast =
          Math.abs(this.speed) >
          this.config.maxSpeed * (isUsingNos ? 0.4 : 0.6);
        if ((isTurning && isFast) || (isBraking && Math.abs(this.speed) > 0.2))
          isSkidding = true;
      }
    }

    if (isSkidding) this.createSkidMarks();
